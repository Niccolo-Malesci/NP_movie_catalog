...
export default {
  ...
  methods: {
    ...
    fetchMovies() {
      ...
      let url = `https://api.themoviedb.org/3/trending/movie/week?page=${page}&api_key=${apiKey}&language=${language}`;
      let url2 = `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&language=${language}&page=${page}${query ? `&query=${query}` : ''}`;
      if (this.searchQuery) {
        url = url2;
      }
      axios
